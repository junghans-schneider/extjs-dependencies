...
function ExtFile(params) {
    ...
    this.resolvePaths = params.resolvePaths;
    this.src = params.src;
...
var Parser = function(options) {
        ...
        if (shouldParseFile(filePath)) {
            ...
            if (classData.classNames.length) {
                ...
                fileInfo = new ExtFile({
                    ...
                    resolvePaths: classData.resolvePaths,
                    src: classData.src,
                    path: filePath
                });
            } else if (classData.requires.length || classData.uses.length || classData.aliasNames || classData.resolvePaths) {
                if (options.verbose) {
    ...
    function getClassData(src) {
        ...
        var parserOptions = extend({
            // For documentation see options of acorn: https://github.com/acornjs/acorn#readme
            comment: true,
            onComment: function(isBlock, text, start, end) {
                if (isBlock) {
                    parseBlockComment(text, output)
                } else {
                    parseLineComment(text, output)
                }
            }
        }, options.parserOptions)

        ast = falafel(src, parserOptions, function (node) {
            switch (node.type) {
                case 'ExpressionStatement':
                    ...
                    } else if (isExtClassMethodCall('ClassManager', 'addNameAlternateMappings', node)) {
                        parseAlternateMappingsCall(node, output);
                    } else if (isExtClassMethodCall('Loader', 'setPath', node)) {
                        parseLoaderSetPathCall(node, output);
                    }
                    break;

                case 'VariableDeclarator':
                    if (isExtDefinition(node)) {
                        parseExtDefinition(node, output);
                    }
                    break;
    ...
    function parseApplicationCall(node, output) {
        ...
        if (p) {
            ...
            // Only look for an appFolder if we have a name
            var appFolder = 'app'; // The default
            p = getObjectProperty(classDef, 'appFolder');
            if (p) {
                appFolder = getPropertyValue(p);
            }
            addResolvePath(output.definedName, path.dirname(_currentFilePath) + '/' + appFolder, output);
        }
    ...
    function parseExtDefinition(node, output) {
        // This is the Ext core file -> Use its path for detecting the path to the Ext source
        // If the file's path is 'path/to/ext/ext-dev.js', the path to the Ext source is 'path/to/ext/src'
        var extSourcePath = path.dirname(_currentFilePath) + '/src';
        addResolvePath('Ext', extSourcePath, output);
    }
    ...
    function getPropertyValueFromObjectOrArray(objectOrArray, name, forceArray) {
        if (objectOrArray.type === 'ObjectExpression') {
            ...
            logger.warn('Expected object or array with property "' + name + '" in ' + getPosition(objectOrArray));
            return [];
        }
    }
    ...
    function isExtClassMethodCall(extClassName, methodName, node) {
        ...
        expr.callee.object.property.name === extClassName &&
        expr.callee.property &&
        expr.callee.property.name === methodName );
    }

    function isExtDefinition(node) {
        // We detect the Ext core file by searching for the declaration: 'var Ext = Ext || {};'
        return node.type === 'VariableDeclarator' &&
                node.parent.type === 'VariableDeclaration' &&
                node.parent.kind === 'var' &&
                node.id.name === 'Ext' &&
                node.init.operator === '||' &&
                node.init.left.name === 'Ext' &&
                node.init.right.type === 'ObjectExpression' &&
                node.init.right.properties.length === 0;
    }

    function getDefinedClassName(node) {
        var firstArgument = node.expression.arguments[0],
            clsNameRaw = firstArgument.value;

        if (typeof clsNameRaw === 'string' && clsNameRaw) {
            return getClassName(clsNameRaw);
        } else if (firstArgument.type === 'BinaryExpression' && firstArgument.right.value === '.$application') {
            // This is the define call in the implementation of `Ext.application`
            // which automatically creates a pseudo class for the application.
            // This call looks lik this: `Ext.define(config.name + ".$application", ...)`
            // -> We ignore it
        } else {
            logger.warn('Cannot determine class name in define call in ' + getPosition(node));
        }
    }

    function parseComment(node, output) {
        if (node.type === 'Line') {
            parseLineComment(node.value, output)
        } else if (node.type === 'Block') {
            parseBlockComment(node.value, output)
        }
    }

    function parseLineComment(commentText, output) {
        var m = AT_REQUIRE_RX.exec(commentText);
        if (m && m[1]) {
        ...
        while (m = DEFINE_RX.exec(commentText)) {
            if (m[1]) {
    ...
    function parseBlockComment(commentText, output) {
        var m;
        while (m = ALTERNATE_CLASS_NAME_RX.exec(commentText)) {
            if (m[1]) {
                addClassNames(output.classNames, m[1]);
            }
        }
    ...
    function addResolvePath(classPrefix, path, output) {
        if (classPrefix && path) {
            if (! output.resolvePaths) {
                output.resolvePaths = {};
            }
            output.resolvePaths[classPrefix] = path;
        }
    }
    ...
    function extrapolateClassNames(basePkgName, baseNms, dependentName) {
        ...
        if (!dependentName) {
            logger.warn('Cannot extrapolate class name without namespace, in ' + getPosition(baseNms));
        }
    ...
    function getPosition(node) {
        var position = node && node.range && node.range[0],
            sourceLength = _currentSource.length;

        if (position && position <= sourceLength) {
            var line = 1,
                lineStart = 0,
                i, char;

            for (i = 0; i < position; i++) {
                char = _currentSource.charAt(i);
                if (char === '\n') {
                    line++;
                    lineStart = i;
                } else if (char === '\r') {
            ...
            var col = position - lineStart;

            return _currentFilePath + ' @ ' + line + ':' + col;
        } else {
