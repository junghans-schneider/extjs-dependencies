...
function resolveDependency(className, sourceFilePath, context) {
    ...
    if (filePath == null) {
        ...
        var fullFilePath = path.resolve(context.options.root, filePath);
        if (!fs.existsSync(fullFilePath)) {
            logger.warn('Couldn\'t find class file for "' + className + '": ' + fullFilePath + ' (found in ' + sourceFilePath + ') - Maybe you should define an alias');
        } else {
            if (context.options.verbose) {
